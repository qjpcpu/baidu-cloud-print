...
<cloudUiDescription name="Hellodoc" defaultScreen="start" xmlns="http://www.hp.com/schemas/imaging/cnx/sip/cloudui/2009/10/31">
  <parameter name="path" type="single"/>
  <screen id="start" showBusy="true">
    <link logical="ok" labelText="<%=ok%>" href="<%=cuiPrint%>/screen/plain_document/path/{path}/access_token/<%=token%>/session_id/<%=session_id%>?<%=pair%>" saveSelection="true"/>
    ...
    %>
    	<link logical="context" labelText="<%=nextPage%>" href="<%=cuiMain%>/screen/plain_document/direction/next/start/<%=sPageStart%>/access_token/<%=token%>/session_id/<%=session_id%>?<%=pair%>" />
    <%
    }
    %>
  </screen>
